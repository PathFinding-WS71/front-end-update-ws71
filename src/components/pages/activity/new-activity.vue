    ...
    <pv-panel class="new-activity-panel" :header="$t('NewActivity')">
      <form @submit="onSubmit" class="activity-form">
        ...
        <div class="p-field">
          ...
          <pv-text-area class="input" v-model="description" id="activity-description" auto-resize rows="3"
                        :placeholder="$t('What is the activity about? Write a description')"/>
        </div>

        <div class="p-field">
          <label for="activity-type" class="label">{{ $t('Type') }}</label>
          <pv-dropdown class="input" v-model="selectedDrop" :options="dropOptions"
                       :placeholder="$t('Select the activity type')"/>
        </div>
        ...
        <div class="p-field">
          ...
          <pv-auto-complete class="input" id="activity-location" v-model="selectedLocation"
                            optionLabel="locationDescription" :suggestions="filteredLocations" @complete="search">
            <template #option="slotProps">
              <div class="flex align-options-center">
                <img :src="slotProps.option.locationImageUrl" :alt="slotProps.option.locationDescription"
                     class="p-mr-2 img-option"/>
